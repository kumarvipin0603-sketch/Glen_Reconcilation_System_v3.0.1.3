...
def apply_branch_team_workbook(uploaded, branch):
    """
    Apply only permitted team fields and only when values actually changed.

    Source/reconciliation columns are ignored, rows from another branch are
    rejected, and an unchanged re-upload is a no-op. Existing team history is
    never downgraded or blanked accidentally.
    """
    ...
    if not all_branches:
        wrong_branch = df[
            df["Branch Code"].fillna("").astype(str).str.strip().ne(str(branch))
        ]
    ...
    for _, r in df.iterrows():
        ...
        row_branch = txt(r.get("Branch Code")) if all_branches else str(branch)
        if not row_branch:
        ...
        # Resolve exact portal/task from hidden System Mapping first.
        targets = []
        if not mapping.empty and {"Order No", "Branch Code"}.issubset(mapping.columns):
            ...
            matched = matched[
                matched["Branch Code"].fillna("").astype(str).str.strip() == row_branch
            ]
        ...
        if not targets:
            with db() as c:
                ...
                """,(row_branch,order_no)).fetchall()

        if not targets:
            unmatched.append(order_no)
            continue

        # Read current MIR values once. Blank cells never clear existing values.
        with db() as c:
            existing_mir = c.execute("""
                SELECT mir_no,mir_date,tei_no,tei_date
                FROM mir_details
                WHERE branch_code=? AND order_no=?
            """,(row_branch,order_no)).fetchone()

        old_mir_no = existing_mir[0] if existing_mir else ""
        old_mir_date = existing_mir[1] if existing_mir else None
        old_tei_no = existing_mir[2] if existing_mir else ""
        old_tei_date = existing_mir[3] if existing_mir else None
        ...
        if mir_changed:
            with db() as c:
                ...
                """,(
                    row_branch, order_no,
                    mir_no or old_mir_no,
                    mir_date or old_mir_date,
                    tei_no or old_tei_no,
                    tei_date or old_tei_date,
        ...
        for portal, oid, task_type in targets:
            with db() as c:
                current = c.execute("""
                    SELECT task_status,working_date,task_completed_date,
                           team_remarks,ticket_raised,raised_date
                    FROM pending_tasks
                    WHERE portal=? AND order_no=? AND task_type=?
                """,(portal,oid,task_type)).fetchone()

            if not current:
                continue

            old_status, old_working, old_completed, old_remarks, old_ticket, old_raised = current
            task_changed = any([
                changed_text(team_remarks, old_remarks),
                changed_text(ticket, old_ticket),
                changed_date(ticket_raised_date, old_raised),
                changed_date(completed_date, old_completed),
            ])

            if not task_changed and not mir_changed:
                continue

            # Completion is sticky. Editing another field on an already completed
            # task must not reopen it. A non-completed task becomes Working when
            # the team makes a real update.
            if completed_date or txt(old_status) == "Completed":
            ...
            with db() as c:
                ...
                """,(
                    new_status,
                    new_status,date.today().isoformat(),
                    new_status,completed_date,date.today().isoformat(),
                ...
                """,(
                    now,"BRANCH TEAM UPDATE",portal,row_branch,
        ...
        # A MIR-only update is still a real saved team change even if no mapped
        # task row needed a task-field update. Count it once in the UI.
...
# ------------------------------------------------------------
    ...
    st.caption(
        ...
        "in the same workbook. Editable fields are MIR No, MIR Date, TEI No, TEI Date, "
    ...
    if all_branch_team_file and st.button(
        ...
        try:
            ...
            st.success(
                f"{n} task record(s) updated permanently across branch codes. "
                "All dashboards now use the saved updates."
            )
            if unmatched:
                st.warning(
